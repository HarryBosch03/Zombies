...
inline void InitializeSimpleLitSurfaceData(float2 uv0, float2 uv1, out SurfaceData outSurfaceData)
{
    ...
    outSurfaceData.albedo *= lerp(1, hash(int3(uv1.xy * 16, 0)).x, _Noise);
    outSurfaceData.albedo = AlphaModulate(outSurfaceData.albedo, outSurfaceData.alpha);
    
    half4 specularSmoothness = SampleSpecularSmoothness(uv0, outSurfaceData.alpha, 0.0, TEXTURE2D_ARGS(_SpecGlossMap, sampler_SpecGlossMap));
    outSurfaceData.metallic = 0.0; // unused
    outSurfaceData.specular = specularSmoothness.rgb;
    outSurfaceData.smoothness = specularSmoothness.a;
    outSurfaceData.normalTS = SampleNormal(uv0, TEXTURE2D_ARGS(_BumpMap, sampler_BumpMap));
    outSurfaceData.occlusion = 1.0;
    outSurfaceData.emission = SampleEmission(uv0, _EmissionColor.rgb, TEXTURE2D_ARGS(_EmissionMap, sampler_EmissionMap));
}
